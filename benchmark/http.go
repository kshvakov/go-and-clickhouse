...
func main() {
	const ddl = `
	CREATE TABLE benchmark_driver_http (
		event_time        DateTime
		, int32_value     Int32
		, int64_value     Int64
		, str_value       String
	...
	`
	const query = `
	INSERT INTO benchmark_driver_http (
		event_time
		, int32_value
		, int64_value
		, str_value
	...
	if _, err := conn.Exec("DROP TABLE IF EXISTS benchmark_driver_http"); err != nil {
		log.Fatal(err)
	}
	...
	for i := 0; i < 10; i++ {
		...
		for i2 := 0; i2 < 500000; i2++ {
			if _, err := block.Exec(
				...
				"str_value",
				"lc_string_value",
				clickhouse.Array([]int8{1, 2, 3, 4, 5}),
			); err != nil {
				log.Fatal(err)
			}
		}
